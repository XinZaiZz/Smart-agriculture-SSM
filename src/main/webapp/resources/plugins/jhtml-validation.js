 ...
 * 
 * ----------------------------------------------------------------------------
 * 参考资料：https://github.com/posabsolute/jQuery-Validation-Engine
 * 
 */
(function($) {
	...
	var methods = {
		...
		/**
		 * Attachs jQuery.validation to form.submit and field.blur events
		 * Takes an optional params: a list of options ie.
		 * jQuery("#formID1").validation('attach', {promptPosition :
		 * "centerRight"});
		 */
		 ...
		 * @param {String}
		 *          type the type of bubble: 'pass' (green), 'load' (black) anything
		 *          else (red)
		 * @param {String}
		 *          possible values topLeft, topRight, bottomLeft, centerRight,
		 *          bottomRight
		 */
		...
		_onSubmitEvent : function() {
			...
			// validate each field
			// (- skip field ajax validation, not necessary IF we will perform an ajax
			// form validation)
		 ...
		 * process all the (field, value) pairs are sent to the server which returns
		 * a list of invalid fields or true
		 * 
		 * @param {jqObject}
		 *          form
		 * @param {Map}
						...
						for (var i = 0; i < json.length; i++) {
							...
							if (errorField.length == 1) {
								...
								if (value[1] == true) {
									...
									if (msg == "" || !msg) {
										// if for some reason, status==true and error="", just close
										// the prompt
										methods._closePrompt(errorField);
									} else {
										// the field is valid, but we are displaying a green prompt
										if (options.rules[msg]) {
											var txt = options.rules[msg].messageOk;
											if (txt)
												msg = txt;
										}
									...
									errorInForm |= true;
									if (options.rules[msg]) {
										var txt = options.rules[msg].message;
										if (txt)
											msg = txt;
									}
									if (options.showPrompts)
		...
		_validateField : function(field, options, skipAjaxValidation) {
			if (!field.attr("id")) {
				field.attr("id", "form-validation-field-" + $.validation.fieldIdCounter);
				++$.validation.fieldIdCounter;
			}
		...
		_getErrorMessage : function(form, field, rule, rules, i, options, originalValidationMethod) {
			...
			// rule.
			// Otherwise if we are doing a function call, make the call and return the
			// object
			// that is passed back.
			...
			// return the custom message instead. Otherwise return the original error
			// message.
			if (errorMsg != undefined) {
		...
		_getCustomErrorMessage : function(field, classes, rule, options) {
			...
			// If there is a validityProp for this rule, check to see if the field has
			// an attribute for it
			...
			// the rule from the id.
			// Otherwise, if we have custom messages for the element's classes, use
			// the first class message we find instead.
		 ...
		 *          user options
		 * @param {bool}
		 *          condRequired flag when method is used for internal purpose in
		 *          condRequired check
		...
		_required : function(field, rules, i, options, condRequired) {
			...
			case "checkbox":
				...
				if (condRequired) {
					if (!field.prop('checked')) {
						return options.rules[rules[i]].messageCheckboxMultiple;
					}
					break;
				}
				...
				if (form.find("input[name='" + name + "']:checked").size() == 0) {
					if (form.find("input[name='" + name + "']:visible").size() == 1)
						return options.rules[rules[i]].messageCheckboxe;
					else
						return options.rules[rules[i]].messageCheckboxMultiple;
				}
				break;
			...
			default:
				...
				if ((!field_val) || (dv_placeholder && field_val == dv_placeholder) || (placeholder && field_val == placeholder)) {
					return options.rules[rules[i]].message;
				}
				break;
			}
		},
		...
		_custom : function(field, rules, i, options) {
			...
			var rule = options.rules[customRule];
			var fn;
			if (!rule) {
			...
			if (rule["regex"]) {
				...
				if (!pattern.test(field.val()))
					return options.rules[customRule].message;
		...
		_equals : function(field, rules, i, options) {
			...
			if (field.val() != $("#" + equalsField).val())
				return options.rules.equals.message;
		},
		/**
		...
		_maxSize : function(field, rules, i, options) {
			...
			if (len > max) {
				var rule = options.rules.maxSize;
				return rule.message + max + rule.message2;
			}
		},
		/**
		...
		_minSize : function(field, rules, i, options) {
			...
			if (len < min) {
				var rule = options.rules.minSize;
				return rule.message + min + rule.message2;
			}
		},
		/**
		...
		_past : function(form, field, rules, i, options) {
			...
			if (vdate > pdate) {
				var rule = options.rules.past;
				if (rule.message2)
					return rule.message + methods._dateToString(pdate) + rule.message2;
				return rule.message + methods._dateToString(pdate);
			}
		},
		/**
		...
		_dateRange : function(field, rules, i, options) {
			...
			if ((!options.firstOfGroup[0].value && options.secondOfGroup[0].value) || (options.firstOfGroup[0].value && !options.secondOfGroup[0].value)) {
				return options.rules[rules[i]].message + options.rules[rules[i]].message2;
			}
		...
		_maxCheckbox : function(form, field, rules, i, options) {
			...
			if (groupSize > nbCheck) {
				options.showArrow = false;
				if (options.rules.maxCheckbox.message2)
					return options.rules.maxCheckbox.message + " " + nbCheck + " " + options.rules.maxCheckbox.message2;
				return options.rules.maxCheckbox.message;
			}
		},
		/**
		...
		_minCheckbox : function(form, field, rules, i, options) {
			...
			if (groupSize < nbCheck) {
				options.showArrow = false;
				return options.rules.minCheckbox.message + " " + nbCheck + " " + options.rules.minCheckbox.message2;
			}
		},
		/**
		...
		_creditCard : function(field, rules, i, options) {
			...
			if (!valid)
				return options.rules.creditCard.message;
		},
		/**
		...
		_ajax : function(field, rules, i, options) {
			...
			var rule = options.rules[errorSelector];
		...
		_showPrompt : function(field, promptText, type, ajaxed, options, ajaxform) {
			...
			// are ebing closed by the engine at the same time
		...
		_condRequired : function(field, rules, i, options) {
			...
			for (idx = (i + 1); idx < rules.length; idx++) {
				 ...
				 * There is logic there for handling all field types, and default value;
				 * so we won't replicate that here Indicate this special use by setting
				 * the last parameter to true so we only validate the dependingField on
				 * chackboxes and radio buttons (#462)
				 */
				if (dependingField.length && methods._required(dependingField, [ "required" ], 0, options, true) == undefined) {
					/*
					 * We now know any of the depending fields has a value, so we can
					 * validate this field as per normal required code
					 */
	...
	$.validation = {
		...
		defaults : {
			...
			showPrompts : true,
			// Should we attempt to validate non-visible input fields contained in the
			// form? (Useful in cases of tabbed containers, e.g. jQuery-UI tabs)
			validateNonVisibleFields : false,
			// ignore the validation for fields with this specific class (Useful in
			// cases of tabbed containers AND hidden fields we don't want to validate)
			...
			rules : {
				required: {
					regex: "none",
					message: "* 此处不可空白",
					messageCheckboxMultiple: "* 请选择一个项目",
					messageCheckboxe: "* 您必须钩选此栏",
					messageDateRange: "* 日期范围不可空白"
				},
				requiredInFunction: {
					func: function(field, rules, i, options) {
						return (field.val() == "test") ? true : false;
					},
					message: "* Field must equal test"
				},
				dateRange: {
					regex: "none",
					message: "* 无效的 ",
				...
				number: {
					// Number, including positive, negative, and floating decimal. credit:
					// orefalo
				...
				// removed or
				// changed to your likings
				ajaxUserCall: {
					url: "ajaxValidateFieldUser",
					// you may want to pass extra data on the ajax call
					extraData: "name=eric",
					message: "* 此名称已被其他人使用",
					messageLoad: "* 正在确认名称是否有其他人使用，请稍等。"
				},
				ajaxUserCallPhp: {
					url: "phpajax/ajaxValidateFieldUser.php",
					// you may want to pass extra data on the ajax call
					extraData: "name=eric",
					// if you provide an "messageOk", it will show as a green prompt
					// when the
					// field validates
					messageOk: "* 此帐号名称可以使用",
					message: "* 此名称已被其他人使用",
					messageLoad: "* 正在确认帐号名称是否有其他人使用，请稍等。"
				},
				ajaxNameCall: {
				...
				dateTimeFormat: {
					...
					message: "* 无效的日期或时间格式",
					message2: "可接受的格式： ",
					message3: "mm/dd/yyyy hh:mm:ss AM|PM 或 ",
					message4: "yyyy-mm-dd hh:mm:ss AM|PM"
				}
			}

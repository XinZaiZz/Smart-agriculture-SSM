 ...
 * 信息发布查询方法
 */	
function getFarm_ycjk_xxfb(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_xxfb/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=信息发布表单]").find("input[name=name]").val() });
		},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增信息发布操作",
	      url: path+"/webview/farm_ycjk_xxfb/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改信息发布操作",
      url: path+"/webview/farm_ycjk_xxfb/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=新增信息发布操作表单]").datagrid();
            ...
            $(".jhtml-tabs").tabs("add",{
                text: "信息发布",
                url: path+"/webview/farm_ycjk_xxfb/xxfb.jsp",
                flush:true,
                close: true
            });
		},
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_xxfb/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

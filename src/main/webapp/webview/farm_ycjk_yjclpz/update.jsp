...
<html lang="zh">
<head>
<title>农业物联网生产系统-修改预警策略配置</title>
</head>
<body>
		...
		<div class="portlet box bordered">
			<div class="portlet-title">
				<div class="caption"  style="color:#45A3CA;">
					<i class="fs icon-pin font-yellow-crusta"></i>修改预警策略配置
				</div>
			</div>
			<div class="portlet-body">
	
				<form name="修改预警策略配置表单" class="form-horizontal" action="${ctx}/farm_ycjk_yjclpz/save"  method="post">
					<div class="form-body">
					...
					<input type="hidden" name="createdBy"/>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label col-md-2"><span class="must-filled">*</span>预警策略名称</label>
									<div class="col-md-9">
										<%--<input type="text" name="name" class="form-control validate[required,maxSize[50]]" />--%>
										<input type="text" name="yjclmc" class="form-control validate[required,maxSize[50]]" />
									</div>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="form-group">
									<label class="control-label col-md-2"><span class="must-filled">*</span>预警等级</label>
									<div class="col-md-9">
									<%--<select name="roleid" class="form-control">--%>
									<%--<select name="yjdj" class="form-control">--%>
										<input type="text" name="yjdj" class="form-control validate[required,maxSize[50]]" />
									</select>
									</div>
								</div>
							</div>
	...
	<script type="text/javascript" src="${ctx}/webview/farm_ycjk_yjclpz/script/farm_ycjk_yjclpz.js" charset="UTF-8"></script>
	<script type="text/javascript">
		...
		$(function() {
			...
			loadRole("form[name=修改预警策略配置表单] select[name=yjdj]");

			if("${param.sid}"){
				$.ajax({
					type: "POST",
					dataType:"json",
					url: "${ctx}/farm_ycjk_yjclpz/getOne",
					data:{sid:"${param.sid}"},
					success: function(data) {
						//加载数据
						load("form[name=修改预警策略配置表单]",data);

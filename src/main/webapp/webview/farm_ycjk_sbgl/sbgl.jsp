...
<body>
 ...
 <%--       <div class="row">
            <div class="col-md-6">--%>
		        <div class="caption" style="color:#45A3CA;">设备管理表单</div>
            <%--</div>--%>
		...
		<div style="text-align: center">
			<div id="bckgDIV">
				<%--<img src="resources/images/sbgl/devbg.png">--%>
				<div id="fwq">
...
<div class="portlet box bordered">
	...
	<div class="portlet-body">
		<table title="设备管理列表" class="table datagrid table-striped table-bordered table-hover order-farm_ycjk_sbgl">
		</table>
	</div>
</div>
	<script type="text/javascript" src="${ctx}/webview/farm_ycjk_sbgl/script/farm_ycjk_sbgl.js" charset="UTF-8"></script>
	<script type="text/javascript">
		...
		$(function() {
			...
			//查询大棚实时监控
			getFarm_ycjk_sbgl("table[title=设备管理列表]");
			...
			$("form[name=设备管理表单] button[title=查询]").on("click", function() {
				var datagrid = $("table[title=设备管理列表]").datagrid();
				datagrid.api().ajax.reload();
			});
			
			$("form[name=设备管理表单] button[title=清空]").on("click", function() {
				clear("form[name=设备管理表单]");
			});
			$("button[title=批量删除]").on("click", function() {
				deleteChartInfo("table[title=设备管理列表]");
			}); 

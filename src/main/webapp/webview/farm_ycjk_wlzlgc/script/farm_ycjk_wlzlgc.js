 ...
 * 操作物流质量过程方法
 */	
function getFarm_ycjk_wlzlgc(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_wlzlgc/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=物流质量过程表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          //{ title:"序号",data: "sid",orderable:true},
					{title: "产品号码",data: "cphm",orderable:true},
					{title: "追溯类别",data: "zslb",orderable:true},
					{title: "操作名称",data: "czmc",orderable:true},
					{title: "操作时间",data: "czsj",orderable:true},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增物流质量过程",
	      url: path+"/webview/farm_ycjk_wlzlgc/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改物流质量过程",
      url: path+"/webview/farm_ycjk_wlzlgc/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=新增物流质量过程表单]").datagrid();
            ...
            $(".jhtml-tabs").tabs("add",{
                text: "物流质量过程",
                url: path+"/webview/farm_ycjk_wlzlgc/wlzlgc.jsp",
                flush:true,
                close: true
            });
		},
    ...
    $(element).ajaxSubmit({
        ...
        success:function(data){
            $.message.alert({
                ...
                callback:function(){
                    if(data.success){
                        var datagrid = $("table[title=修改物流质量过程表单]").datagrid();
                        datagrid.api().ajax.reload();
                    }
                }
            });
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_wlzlgc/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,
							callback:function(){
								if(data.success){
									var datagrid = $('table[title=预警策略配置列表]').datagrid();
									datagrid.api().ajax.reload();
								}
							}
						});
				...
				$.ajax({
					...
					success: function(data) {
						$.message.alert({
							...
							callback:function(){
								if(data.success){
									var datagrid = $('table[title=物流质量过程列表]').datagrid();
									datagrid.api().ajax.reload();
								}
							}
						});
...
function loadRole(element,params){
	$.ajax({
		type: "POST",
		dataType:"json",
		url: path+"/farm_ycjk_wlzlgc/find",
		success: function(data) {
			$(element).empty();

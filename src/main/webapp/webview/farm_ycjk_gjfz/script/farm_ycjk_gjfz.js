 ...
 * 查询告警阀值方法
 */	
function getFarm_ycjk_gjfz(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_gjfz/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=告警阀值查询表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          { title:"空气温度",data: "kqwd",orderable:true},
					{title: "空气湿度",data: "kqsd",orderable:true},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增告警阀值",
	      url: path+"/webview/farm_ycjk_gjfz/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改告警阀值",
      url: path+"/webview/farm_ycjk_gjfz/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=告警阀值设置表单]").datagrid();
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_gjfz/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,
							callback:function(){
								if(data.success){
									var datagrid = $('table[title=告警阀值列表]').datagrid();
									datagrid.api().ajax.reload();
								}
							}
						});

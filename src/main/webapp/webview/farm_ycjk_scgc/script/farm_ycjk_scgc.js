...
function getFarm_ycjk_scgc(element){
	$(element).datagrid( {
		...
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=生产过程表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          //{ title:"序号",data: "sid",orderable:true},
					{title: "生产过程描述",data: "scgcms",orderable:true},
					{title: "备注信息",data: "bzxx",orderable:true},
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=新增生产过程表单]").datagrid();
    ...
    $(element).ajaxSubmit({
        ...
        success:function(data){
            $.message.alert({
                ...
                callback:function(){
                    if(data.success){
                        var datagrid = $("table[title=修改生产过程表单]").datagrid();
                        datagrid.api().ajax.reload();
                    }
                }
            });

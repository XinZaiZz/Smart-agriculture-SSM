 ...
 * 查询大棚实时监控方法
 */	
function getFarm_ycjk_ssjk(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_ssjk/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=大棚实时监控查询表单]").find("input[name=name]").val() });
		},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增实时监控",
	      url: path+"/webview/farm_ycjk_ssjk/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改用户",
      url: path+"/webview/farm_ycjk_ssjk/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_ssjk/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

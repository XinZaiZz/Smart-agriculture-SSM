...
<body>
		...
		<div class="portlet box bordered">
			<div class="portlet-title">
				<div class="caption"  style="color:#45A3CA;">
					<i class="fs icon-pin font-yellow-crusta"></i>修改用户信息
				</div>
			</div>
			<div class="portlet-body">
	
				<form name="修改用户信息表单" class="form-horizontal" action="${ctx}/userInfo/save"  method="post">
					<div class="form-body">
	...
	<script type="text/javascript">
		...
		$(function() {
			//查询角色
			loadRole("form[name=修改用户信息表单] select[name=roleid]");
			
			if("${param.sid}"){
				$.ajax({
					type: "POST",
					dataType:"json",
					url: "${ctx}/userInfo/getOne",
					data:{sid:"${param.sid}"},
					success: function(data) {
						//加载数据
						load("form[name=修改用户信息表单]",data);
			...
			$("form[name=修改用户信息表单] button[title=保存]").on("click",function(){
				save("form[name=修改用户信息表单]");
			});

			$("form[name=修改用户信息表单] button[title=关闭]").on("click", function() {

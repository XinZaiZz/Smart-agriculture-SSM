 ...
 * 操作设备管理方法
 */	
function getFarm_ycjk_sbgl(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_sbgl/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=设备管理表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          //{ title:"主键ID",data: "sid",orderable:true},
		          	{ title:"设备ID",data: "sbid",orderable:true},
					{title: "设备名称",data: "sbmc",orderable:true},
					{title: "设备类型",data: "sblx",orderable:true},
					{title: "投入情况",data: "trqk",orderable:true},
					{title: "设备状态",data: "sbzt",orderable:true},
					{title: "所在位置",data: "szwz",orderable:true},
					{title: "其他信息",data: "qtxx",orderable:true},
        ...
        columnDefs: [{
        	...
        	"createdCell": function (td, cellData, rowData, row, col){
                ...
                $(td).html("<a href=\"javascript:void(0)\" onclick=\"deleteById({'sid':'"+rowData.sid+"'})\" title=\"删除\" class=\"label label-sm btn btn-info margin-right-5\">删除</a>");
    	},
        	targets: [-1]
        }]
} );
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增设备管理",
	      url: path+"/webview/farm_ycjk_sbgl/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改设备管理",
      url: path+"/webview/farm_ycjk_sbgl/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=设备管理表单]").datagrid();
            ...
            $(".jhtml-tabs").tabs("add",{
                text: "设备管理",
                url: path+"/webview/farm_ycjk_sbgl/sbgl.jsp",
                flush:true,
                close: true
            });
		},
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_sbgl/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

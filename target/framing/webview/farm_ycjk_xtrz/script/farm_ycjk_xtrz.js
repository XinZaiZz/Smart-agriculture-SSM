 ...
 * 操作系统日志方法
 */	
function getFarm_ycjk_xtrz(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_xtrz/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=系统日志表单]").find("input[name=name]").val() });
			aoData.push({name: "startDate", value: $("form[name=系统日志表单]").find("input[name=startDate]").val() });
			aoData.push({name: "endDate", value: $("form[name=系统日志表单]").find("input[name=endDate]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          //{ title:"主键ID",data: "sid",orderable:true},
					...
					{title: "日志类型",data: "rzlx",orderable:true},
					{title: "日志内容",data: "rznr",orderable:true},
					{title: "记录时间",data: "jlsj",orderable:true},
					{title: "操作人员",data: "czry",orderable:true}
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增系统日志",
	      url: path+"/webview/farm_ycjk_xtrz/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改系统日志",
      url: path+"/webview/farm_ycjk_xtrz/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=新增系统日志表单]").datagrid();
            ...
            $(".jhtml-tabs").tabs("add",{
                text: "系统日志",
                url: path+"/webview/farm_ycjk_xtrz/xtrz.jsp",
                flush:true,
                close: true
            });
		},
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_xtrz/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

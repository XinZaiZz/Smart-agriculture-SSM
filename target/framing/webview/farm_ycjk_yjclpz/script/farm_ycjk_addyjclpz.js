 ...
 * 预警配置策略查询方法
 */	
function getFarm_ycjk_yjclpz(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_yjclpz/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=预警策略配置表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          //{ title:"序号",data: "sid",orderable:true},
					{title: "预警策略名称",data: "yjclmc",orderable:true},
					{title: "预警等级",data: "yjdj",orderable:true},
					{title: "作用时间",data: "zysj",orderable:true},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增预警策略配置",
	      url: path+"/webview/farm_ycjk_yjclpz/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改预警策略配置",
      url: path+"/webview/farm_ycjk_yjclpz/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=新增预警策略配置表单]").datagrid();
            ...
            $(".jhtml-tabs").tabs("add",{
                text: "预警策略配置",
                url: path+"/webview/farm_ycjk_yjclpz/yjclpz.jsp",
                flush:true,
                close: true
            });
		},
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_yjclpz/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,
...
function loadRole(element,params){
	$.ajax({
		type: "POST",
		dataType:"json",
		url: path+"/farm_ycjk_yjclpz/find",
		success: function(data) {
			$(element).empty();

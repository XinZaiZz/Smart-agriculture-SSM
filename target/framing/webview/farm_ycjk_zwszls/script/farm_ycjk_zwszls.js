 ...
 * 作物生长历史查询方法
 */	
function getFarm_ycjk_zwszls(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_zwszls/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=作物生长历史表单]").find("input[name=name]").val() });
            aoData.push({name: "startDate", value: $("form[name=作物生长历史表单]").find("input[name=startDate]").val() });
            aoData.push({name: "endDate", value: $("form[name=作物生长历史表单]").find("input[name=endDate]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          { title:"序号",data: "sid",orderable:true},
					{title: "叶面积指数(LAI)",data: "ymjzs",orderable:true},
					{title: "归一化指数(NDVI)",data: "gyhzs",orderable:true},
					{title: "叶片含水量",data: "yphsl",orderable:true},
					{title: "叶绿素含量",data: "ylshl",orderable:true},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增实时监控",
	      url: path+"/webview/farm_ycjk_zwszls/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改用户",
      url: path+"/webview/farm_ycjk_zwszls/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_zwszls/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

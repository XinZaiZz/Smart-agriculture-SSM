...
function getUserInfo(element){
	$(element).datagrid( {
		ajaxSource: path+"/userInfo/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=用户信息查询表单]").find("input[name=name]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          { title:"用户名",data: "name",orderable:true},
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增用户",
	      url: path+"/webview/userInfo/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改用户",
      url: path+"/webview/userInfo/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}

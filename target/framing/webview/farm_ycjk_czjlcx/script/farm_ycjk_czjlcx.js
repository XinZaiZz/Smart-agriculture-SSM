 ...
 * 操作记录查询方法
 */	
function getFarm_ycjk_czjlcx(element){
	$(element).datagrid( {
		ajaxSource: path+"/farm_ycjk_czjlcx/find",
		ajaxDataProp:"rows",
		serverMethod:"POST",
		sort: false,
		fnServerParams: function (aoData) {  
			aoData.push({name: "name", value: $("form[name=操作记录查询表单]").find("input[name=name]").val() });
            aoData.push({name: "startDate", value: $("form[name=操作记录查询表单]").find("input[name=startDate]").val() });
            aoData.push({name: "endDate", value: $("form[name=操作记录查询表单]").find("input[name=endDate]").val() });
		},
		columns: [{data:"sid",title:"<input type='checkbox' class='group-checkable' data-set='table[title="+$(element).attr("title")+"] .checkboxes' />"},
		          ...
		          { title:"设备名称",data: "sbmc",orderable:true},
					{title: "操作指令",data: "czzl",orderable:true},
					{title: "处理状态",data: "clzt",orderable:true},
					{title: "处理时间",data: "clsj",orderable:true},
					{title: "备注",data: "bz",orderable:true},
					{title: "操作员名",data: "czym",orderable:true}
...
function add(){
	$(".jhtml-tabs").tabs("add",{
	      text: "新增操作记录查询",
	      url: path+"/webview/farm_ycjk_czjlcx/add.jsp",
	      flush:true,
	      close: true
	    });
}
...
function toUpdate(params){
	$(".jhtml-tabs").tabs("add",{
      text: "修改操作记录查询",
      url: path+"/webview/farm_ycjk_czjlcx/update.jsp?sid="+params.sid,
      flush:true,
      close: true
    });
}
	...
	$(element).ajaxSubmit({
		...
		success:function(data){
			$.message.alert({
				...
				callback:function(){
					if(data.success){
						var datagrid = $("table[title=操作记录查询列表]").datagrid();
		...
		$.message.confirm('您确认想要删除记录吗？',function(r){    
	    	if(r){
				$.ajax({
					...
					url: path+"/farm_ycjk_czjlcx/delete",
					data:"[{\"sid\":\""+sid+"\"}]",
					success: function(data) {
						$.message.alert({
							message:data.message,

...
<body>
 ...
 <%--       <div class="row">
            <div class="col-md-6">--%>
		        <div class="caption" style="color:#45A3CA;">物流质量过程表单</div>
            <%--</div>--%>
...
<div class="portlet box bordered">
	...
	<div class="portlet-body">
		<table title="物流质量过程列表" class="table datagrid table-striped table-bordered table-hover order-farm_ycjk_wlzlgc">
		</table>
	</div>
</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			//查询大棚实时监控
			getFarm_ycjk_wlzlgc("table[title=物流质量过程列表]");
			...
			$("form[name=物流质量过程表单] button[title=查询]").on("click", function() {
				var datagrid = $("table[title=物流质量过程列表]").datagrid();
				datagrid.api().ajax.reload();
			});
			
			$("form[name=物流质量过程表单] button[title=清空]").on("click", function() {
				clear("form[name=物流质量过程表单]");
			});
			$("button[title=批量删除]").on("click", function() {
				deleteChartInfo("table[title=物流质量过程列表]");
			}); 
